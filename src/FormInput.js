...
import { rateDerivation } from "./financialUtil";
...
const FormInput = (props) => {
  ...
  const compute = () => {
    ...
    const interestRateDuePeriod = rateDerivation(paymentPeriod, monthlyInterestRate);
    let paymentsList = [];

    if (interestType === "Basit Faiz") {
      paymentsList = simpleInterestCalculation(
        principalValue,
        numberOfPayment,
        interestRateDuePeriod,
        kkdf,
        bsmv
      );
    } else {
      paymentsList = compoundInterestCalculation(
        principalValue,
        numberOfPayment,
        interestRateDuePeriod,
        kkdf,
